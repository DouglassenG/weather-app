...
export async function GET(request: NextRequest) {
  ...
  if (!city && !lat) {
    return NextResponse.json(
      { message: "Informe uma cidade ou coordenadas" },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    if (lat && lon) {
      ...
      resolvedName = locationName || "Desconhecida";
      resolvedCountry = locationCountry || "";
      resolvedState = locationState || undefined;
    } else {
      let searchCity = city!;
      let stateFilter = "";
      ...
      if (searchCity.includes(",")) {
        ...
        searchCity = parts[0];
        stateFilter = parts[1]?.toLowerCase() || "";
      }

      const geoUrl = `https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(searchCity)}&count=10&language=pt`;
      ...
      if (stateFilter) {
        ...
        const fullStateName = STATE_MAP[stateFilter] || stateFilter;

        filteredResults = geoData.results.filter((result) => {
          const admin = result.admin1?.toLowerCase() || "";
          return admin.includes(fullStateName) || admin.includes(stateFilter);
        });

        if (filteredResults.length === 0) {
          filteredResults = geoData.results;
        }
      }

      if (filteredResults.length > 1 && !stateFilter) {
        return NextResponse.json({
          type: "multiple",
      ...
      const location = filteredResults[0];
      latitude = location.latitude;
      longitude = location.longitude;
      resolvedName = location.name;
      resolvedCountry = location.country_code;
